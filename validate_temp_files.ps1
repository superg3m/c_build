...
if ($differences) {
    if ($current_filename -eq "bootstrap.ps1") {
        WriteHost "Template content for '$current_filename' is out of sync; running ./c_build/bootstrap.ps1" -ForegroundColor yellow
        ./c_build/bootstrap.ps1
    } else {
        WriteHost "Template content for '$current_filename' is out of sync; running ./bootstrap.ps1" -ForegroundColor yellow
        ./bootstrap.ps1
    }
