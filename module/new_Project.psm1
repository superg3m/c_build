...
class Project {
    ...
    [bool]ProjectIsBuilt([Project] $project) {
        foreach ($build_procedure in $this.build_procedures) {
            if ($build_procedure.IsBuilt() -eq $false) {
                return $false
            }
        }
    ...
    [void]BuildProject() {
        Write-Host "|--------------- Started Building $($this.name) ---------------|" -ForegroundColor Blue
        Write-Host "Compiler: $($this.compiler_type)" -ForegroundColor Blue
        $timer = [Diagnostics.Stopwatch]::new()
        $timer.Start()

        $this.BuildProjectDependencies()
        $this.BuildProcedures
